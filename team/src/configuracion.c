...
{
    uint32_t socketServer = crear_conexion(values.ip_broker, values.puerto_broker);

    //uno por cada cola de mensajes
    //send_order_subscription_queue_message(char **mensaje, uint32_t socketServer, t_log * obligatory_logger);
